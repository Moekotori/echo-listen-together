...
| `RECONNECT_SECONDS` | `20` | Session retention after a short disconnect |
| `EMPTY_ROOM_SECONDS` | `60` | Cleanup delay for remaining empty rooms; explicit host departure ends a room immediately |
...
Default audio uses 48 kHz stereo Opus in 20 ms packets. At 128 kbps, 200 listener streams require approximately 25.6 Mbps of audio payload, plus protocol overhead and headroom. Slow receivers with more than 16 KiB of queued audio are disconnected to avoid unbounded buffering. TLS WebSocket uses TCP, so packet loss can disrupt real-time delivery. Carrier routes, peak hours, sustained load, and two-account Steam behavior require separate validation.

See the [2026-09-09 verification record](docs/verification-2026-09-09.md) for native audio, reconnect results, and scope limitations.

## Privacy and development

Only the explicitly selected server receives nicknames, room information, optional track titles, and audio after sharing is enabled. The client does not upload the library, file paths, artwork, lyrics, or playback history. Server operators can access data passing through their server. Room passwords use salted scrypt and remain in memory; invitations and logs are bounded. The service does not record audio, passwords, or invitations and does not enable access logs. Share only content you have the right to transmit.
