...
**音频固定为 256 kbps，不支持切换音质。** 房主只上传一条 Opus 流；客户端和服务器需配套更新，旧房主的多档广播会收到升级错误。
...
| `RECONNECT_SECONDS` | `60` | 短期断线会话保留时间 |
| `EMPTY_ROOM_SECONDS` | `60` | 连续无人在线的房间回收时间；离线成员仍在重连宽限期时不提前清理，有人重新上线会取消计时。房主在线但暂停或独处不算空房；房主主动离开或重连期限到期仍按原规则结束房间 |
...
音频固定为 256 kbps、48 kHz 双声道 Opus、20 ms 包。200 路听众的纯音频约 51.2 Mbps，另需协议开销和余量。慢接收端待发音频超过 16 KiB 时丢弃新音频，持续拥塞 10 秒后断开，避免无界排队。TLS WebSocket 基于 TCP，丢包会影响实时性；真实三网、晚高峰、长期负载与 Steam 双账号需分别验收。

本次服务端同步见 [2026-09-10 验证记录](docs/verification-2026-09-10.md)。早期原生音频及重连检查见 [2026-09-09 验证记录](docs/verification-2026-09-09.md)，两份记录均注明了验证边界。

## 隐私与开发

只向用户明确连接的服务器发送昵称、房间信息、主动开启共享后的音频，以及兼容客户端提供的当前歌曲标签、受限封面缩略图、歌词和播放时钟。聊天和正在输入状态只转发给同房成员。不上传整个曲库、文件路径或播放历史；当前歌曲数据仅驻留内存，不保存音频或元数据历史。服务器运营者能接触经过服务器的数据。房间密码使用随机盐 scrypt，仅驻留内存；邀请和日志有界；服务不记录音频、密码或邀请，不启用访问日志。只分享有权传输的内容。
...
服务端会记录连接关闭原因、首包超时、断流/恢复和校验/限流/拥塞计数，不记录音频内容、聊天正文、密码或邀请。
需要排错时，在服务器项目目录运行（仅此报告工具需要宿主机 Python 3）：
