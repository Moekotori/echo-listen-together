...
test('keeps one current cover, ignores lyrics and unknown data, deduplicates and bounds update rate',()=>{
 ...
 updateArtwork(room,input,broadcast,0);assert.equal(calls,1);assert.equal(room.track.lyrics,null);assert.equal(room.track.filePath,undefined);
