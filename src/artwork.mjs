import { sanitizeListeningTrackTechnical } from './track-technical.mjs';
import { readSharedLyrics, readMediaClock } from './lyrics.mjs';
// One bounded current programme per room; no disk cache or external URLs.
...
export function updateArtwork(room, input, broadcast, now = performance.now()) {
  ...
  const track = { technical: sanitizeListeningTrackTechnical(input.track.technical), title: tag(input.track.title), artist: tag(input.track.artist), album: tag(input.track.album), cover: readArtwork(input.track.cover), lyrics: readSharedLyrics(input.track.lyrics), clock: readMediaClock(input.track.clock) };
