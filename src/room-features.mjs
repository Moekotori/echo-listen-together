...
export function qualityIndex(room, peer) {
  return room.multiQuality ? Math.max(0, qualities.indexOf((room.hostId === peer.id ? room.defaultQuality : peer.quality ?? room.defaultQuality) || 128)) : 0;
}
export function roomForPeer(snapshot, room, peer) {
  ...
  return { ...snapshot, qualities: room.multiQuality ? qualities : [128],
    quality: qualities[index], preferredQuality: (room.hostId === peer.id ? room.defaultQuality : peer.quality ?? room.defaultQuality) || 128,
...
export function selectQuality(rooms, peer, input) {
  ...
  if (room.hostId === peer.id) { room.defaultQuality = input.value; rooms.broadcast(room); return true; }
